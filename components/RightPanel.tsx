...
import { Trophy, MapPin, Lightbulb, Send, Lock, Globe, Navigation, ChevronDown, ChevronUp, CheckCircle, Info, Languages, Minus, User, MessageSquare, Volume2, VolumeX, MessageCircle, Eye, XCircle } from 'lucide-react';
...
export function RightPanel({
  ...
  return (
    <div className="absolute top-0 right-0 bottom-0 w-80 md:w-96 z-10">
      <Card className="h-full rounded-none border-l-4 border-[#FFC832] bg-gradient-to-b from-slate-900/98 to-slate-800/98 backdrop-blur-sm shadow-2xl flex flex-col">
        {/* Header */}
        <div className="p-4 bg-gradient-to-br from-slate-800 to-slate-900 border-b-2 border-[#FFC832]/30">
          ...
          <div className="space-y-2">
            {/* Timer */}
            <div className={`rounded-lg p-3 flex items-center justify-between border-2 ${
              roundTimer <= 30 ? 'bg-red-500 border-red-600 animate-pulse' :
              roundTimer <= 60 ? 'bg-orange-500 border-orange-600' :
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white font-medium">Time Remaining</span>
              </div>
            ...
            <div className="bg-[#FFC832] rounded-lg p-3 flex items-center justify-between border-2 border-[#E6B42E]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-900 font-medium">Total Points</span>
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-emerald-400 text-xs">1000 points per round</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-blue-400 text-xs">Up to +500 points (faster = more points)</p>
                          </div>
                        </div>

                        {/* Penalties */}
                        <div className="pt-2 border-t border-slate-700 space-y-2">
                          <p className="text-slate-300 text-xs font-semibold">Penalties:</p>
                          <div className="space-y-1.5 pl-2">
                            <p className="text-red-400 text-xs">• Wrong Guess: -75 points</p>
                            <p className="text-red-400 text-xs">• Each Hint: -100 points</p>
                            <p className="text-red-400 text-xs">• Each Translation: -50 points</p>
                          </div>
                        </div>
                        ...
                        <div className="pt-2 border-t border-slate-700">
                          <p className="text-[#FFC832] text-xs flex items-center gap-1">
                            ...
                            <span>Tip: Answer quickly without hints for maximum points!</span>
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <span className="text-2xl text-slate-900 tabular-nums font-bold">{totalPoints.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        {reviewMode && (
          <div className="px-4 pt-4 pb-2">
            <div className="bg-gradient-to-r from-blue-900/50 to-purple-900/50 border-2 border-blue-400/30 rounded-full px-4 py-2.5 flex items-center gap-2 shadow-lg backdrop-blur-sm">
              <div className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-500/30 flex items-center justify-center border border-blue-400/40">
                <Eye className="w-3.5 h-3.5 text-blue-300" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs text-blue-200 leading-relaxed">
                  Reviewing this ride – your answer is already locked in.
                </p>
              </div>
        ...
        <div className="px-4 pt-4">
          <div className="p-3 bg-slate-800/50 rounded-lg border-2 border-[#FFC832]/40 shadow-lg">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Lightbulb className="w-4 h-4 text-[#FFC832]" />
                <span className="text-white text-sm">Hints Available</span>
                <Badge variant="outline" className="bg-slate-900 border-[#FFC832]/60 text-[#FFC832] text-xs">
                  {hintsUsed}/{maxHints}
                </Badge>
              </div>
            </div>
                    ...
                    {conversationMessages.length === 0 ? (
                      ...
                      conversationMessages.map((message) => {
                        ...
                        return (
                          <div
                            key={message.id}
                            className={`flex items-start gap-2 ${!isDriver ? 'flex-row-reverse' : ''}`}
                          >
                            {/* Avatar */}
                            <div className="flex-shrink-0">
                              <Avatar className={`w-8 h-8 border-2 shadow-lg ${
                                ...
                                >
                                  {isDriver ? getInitials(driverName) : <User className="w-4 h-4" />}
                                </AvatarFallback>
                              </Avatar>
                            </div>

                            {/* Message Bubble */}
                            <div className={`flex-1 min-w-0 ${!isDriver ? 'flex flex-col items-end' : ''}`}>
                              <div className={`mb-1 flex items-center gap-1.5 ${!isDriver ? 'flex-row-reverse' : ''}`}>
                              ...
                              <div
                                ...
                                style={
                                  isDriver
                                    ? {
                                        backgroundColor: (message.color || '#10b981') + '20',
                                        borderColor: (message.color || '#10b981') + '40',
                                        borderLeftColor: voiceEnabled ? '#10b981' : undefined
                                      }
                                    : {
                              ...
                              >
                                ...
                                {/* Original Text */}
                                <div>
                                  <p className={`text-white leading-relaxed text-xs ${!isDriver ? 'text-right' : ''}`}>
                                    {message.text}
                                  </p>
                                  {showTranslation && hasTranslation && (
                                    <p className={`text-slate-300 text-xs leading-relaxed italic mt-1.5 ${!isDriver ? 'text-right' : ''}`}>
                                      {message.translation}
                                    </p>
                                  )}
                                </div>
        ...
        {reviewMode ? (
          ...
          <div className="p-4 bg-gradient-to-b from-slate-900/50 to-slate-900/80">
            ...
            <p className="text-xs text-slate-400 mb-3">
              You've completed this ride. Review the conversation or continue your journey.
            </p>
                    ...
                    <div className="space-y-3 text-xs">
                      <div className="bg-[#FFC832]/10 p-3 rounded border border-[#FFC832]/30">
                        <p className="text-slate-300 mb-1.5">
                          <span className="text-[#FFC832]">🎯 Perfect Score:</span>
                        </p>
                      ...
                      <div className="bg-emerald-900/20 p-2.5 rounded border border-emerald-700/30">
                        <p className="text-emerald-200 text-xs">
                          <strong>💬 Conversations are free!</strong> Chat with your driver as much as you want.
                        </p>
                      </div>
                    </div>
                  </div>
